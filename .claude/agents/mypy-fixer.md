...
Fixes mypy type errors and eliminates `Any` usage in the mutt-oauth2 project.
...
1. Fix all mypy errors reported by `uv run mypy mutt_oauth2`
...
Place new TypedDict classes in `mutt_oauth2/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`mutt_oauth2/typing.py` when shared.
...
1. Run `uv run mypy mutt_oauth2` and capture all errors.
...
4. Run `uv run mypy mutt_oauth2` again to verify fixes.
