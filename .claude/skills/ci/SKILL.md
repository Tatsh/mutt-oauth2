...
If any changed files are under `mutt_oauth2/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `mutt_oauth2/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `mutt_oauth2/commands/main.py`,
...
For Python files, strip the `mutt_oauth2/` prefix and replace `/` with `.` (like module imports).

- Python file `mutt_oauth2/media.py` → `media:`.
- Multiple files under `mutt_oauth2/commands/` → `commands:`.
- Single command file `mutt_oauth2/commands/admin.py` → `commands.admin:`.
